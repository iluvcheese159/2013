...
export default function TopBar() {
  ...
  return (
    <header
      data-testid="top-bar"
      className={`fixed top-0 right-0 z-30 backdrop-blur-xl border-b border-border h-14 flex items-center px-6 transition-all bg-background/80 ${
        ...
        {user ? (
          <>
            {!user.is_pro && (
              ...
              >
                <Sparkles className="h-3.5 w-3.5 mr-1.5" /> Hyperspace
              </Button>
            )}
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-64">
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">
                    ...
                    {user.is_pro && (
                      <span className="text-[9px] font-tech uppercase tracking-wider px-2 py-0.5 bg-accent/10 text-accent border border-accent/30 rounded-full">
                        <Sparkles className="inline h-2.5 w-2.5 mr-0.5" />Hyperspace
                      </span>
                    )}
                    {user.is_seller && (
                ...
                {!user.is_pro && (
                  <DropdownMenuItem data-testid="menu-pro" onClick={() => navigate("/pro")}>
                    <Sparkles className="h-4 w-4 mr-2 text-accent" /> Upgrade to Hyperspace
                  </DropdownMenuItem>
                )}
                {!user.is_admin && (
