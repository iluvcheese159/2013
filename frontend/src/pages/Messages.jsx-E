...
export default function Messages() {
  ...
  const openConstellationBuilder = (club) => {
    if (club.club_creator_owner_id === user?.user_id) {
      setEditingClubId(club.chat_id);
      setBuilderDialogOpen(true);
    }
  };

  // Check if preview window has expired (10 minutes)
  useEffect(() => {
    if (!isPreviewMode || !previewStartTime) return;
  ...
  }, [isPreviewMode, previewStartTime]);
  ...
  const openClub = async (club) => {
    ...
    const isMember = myClubs.some((c) => c.chat_id === club.chat_id);
  ...
  return (
    <div data-testid="messages-page" className="pt-14 h-screen flex">
      <aside className="w-80 border-r border-border bg-card overflow-y-auto flex flex-col">
        ...
        {tab === "clubs" && (
          <div className="flex-1">
            ...
            {myClubs.map((c) => (
              <div key={`my-${c.chat_id}`} className="px-3 py-2 border-b border-border">
                <div className="flex items-start gap-2">
                  <button onClick={() => openClub(c)} className="flex-1 text-left">
                    <div className="mb-2">
                      {constellationData[c.chat_id] && constellationData[c.chat_id].stars?.length > 0 ? (
                        <div className="w-full aspect-square bg-black rounded-lg overflow-hidden">
                          <svg className="w-full h-full">
                            {constellationData[c.chat_id].lines?.map((line) => {
                              const fromStar = constellationData[c.chat_id].stars.find((s) => s.id === line.from);
                              const toStar = constellationData[c.chat_id].stars.find((s) => s.id === line.to);
                            ...
                            {constellationData[c.chat_id].stars?.map((star) => (
                              <circle
                                key={star.id}
                                cx={`${star.x}%`}
                                cy={`${star.y}%`}
                  ...
                  {c.club_creator_owner_id === user?.user_id && (
                    <button
                      onClick={() => openConstellationBuilder(c)}
                      className="h-7 w-7 border border-border rounded-xl inline-flex items-center justify-center hover:border-primary"
      ...
      <main className="flex-1 flex flex-col">
              ...
              {isPreviewMode ? (
                <div className="flex-1 text-center">
                  <p className="text-sm text-muted-foreground mb-2">
                    Preview mode expires in {Math.max(0, Math.ceil((10 * 60 * 1000 - (Date.now() - previewStartTime)) / 60000))} minutes
                  </p>
                  <Button onClick={() => joinClub(activeClub, false)} className="bg-primary hover:bg-primary/90 rounded-xl font-tech text-xs uppercase tracking-wider">
        ...
        {sidebarOpen && tab === "clubs" && activeClub && (
          <div className="w-80 border-l border-border bg-card overflow-y-auto">
            <div className="p-4 border-b border-border">
              <button onClick={() => setSidebarOpen(false)} className="text-muted-foreground hover:text-foreground mb-2">
                <ArrowLeft className="h-4 w-4" />
              </button>
              <h3 className="font-display text-lg font-medium">{activeClub.club_name}</h3>
              <div className="text-[10px] font-tech uppercase tracking-wider text-muted-foreground">Club Perks</div>
            </div>
            <div className="p-4 space-y-4">
              {activeClub.is_charge_subscription_enabled ? (
                <div className="bg-primary/10 border border-primary/20 rounded-xl p-4">
                  <div className="text-sm font-medium mb-2">Subscription Required</div>
                  <div className="text-[#F59E0B] font-tech text-lg mb-2">${Number(activeClub.club_entry_monthly_price || 0).toFixed(2)} / month</div>
                  <div className="text-xs text-muted-foreground space-y-1">
                    <div>✓ Full message history</div>
                    <div>✓ Unlimited clubs</div>
                    <div>✓ Priority support</div>
                  </div>
                  <Button onClick={() => joinClub(activeClub, true)} className="w-full mt-3 bg-primary hover:bg-primary/90 rounded-xl font-tech text-xs uppercase tracking-wider">
                    Subscribe & Join
                  </Button>
                </div>
              ) : (
                <div className="bg-secondary/30 border border-border rounded-xl p-4">
                  <div className="text-sm font-medium mb-2">Free Access</div>
                  <div className="text-xs text-muted-foreground space-y-1">
                    <div>✓ Basic message access</div>
                    <div>✓ Limited history (50 messages)</div>
                    <div>✓ Up to 5 free clubs</div>
                  </div>
                  {isPreviewMode && (
                    <Button onClick={() => joinClub(activeClub, false)} className="w-full mt-3 bg-primary hover:bg-primary/90 rounded-xl font-tech text-xs uppercase tracking-wider">
                      Join Free Club
                    </Button>
                  )}
                </div>
              ...
              {/* Bob in chat area (simpler version) */}
              <div className="border border-border rounded-xl p-4 bg-black/5">
                <div className="text-[10px] font-tech uppercase tracking-wider text-muted-foreground mb-2">AI Helper</div>
                <div className="text-xs text-muted-foreground">
                  Bob is here to help! Ask him anything about this club or Print Cosmos.
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Constellation Builder Dialog */}
      <Dialog open={builderDialogOpen} onOpenChange={setBuilderDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle className="font-display text-2xl">Build Your Constellation</DialogTitle>
          </DialogHeader>
          <div className="py-4">
            <ConstellationBuilder
              onSave={handleSaveConstellation}
              initialData={constellationData[editingClubId]}
            />
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setBuilderDialogOpen(false)} className="rounded-xl font-tech text-xs uppercase tracking-wider">Cancel</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
