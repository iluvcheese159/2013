...
import { Shield, Scale, Medal, Ban, Trash2, Award, FileText, Lock, ExternalLink } from "lucide-react";
...
export default function OwnerControlHub() {
  ...
  const [medalTarget, setMedalTarget] = useState(null);
  const [medalId, setMedalId] = useState("");
  const [medalLabel, setMedalLabel] = useState("");
  ...
  const suspendUser = async () => {
    ...
    try {
      await api.post(`/owner/users/${suspendTarget.user_id}/suspend`, {
        duration_days: suspendDays,
        reason_text: suspendReason,
      });
  ...
  const awardMedal = async () => {
    if (!medalTarget || !medalId.trim() || !medalLabel.trim()) {
      toast.error("badge_id and label are required");
      return;
    }
    setMedalSubmitting(true);
    try {
      await api.post(`/owner/users/${medalTarget.user_id}/award-medal`, {
        badge_id: medalId.trim(),
        label: medalLabel.trim(),
        description: medalDesc.trim(),
      });
      toast.success("Exclusive medal awarded");
      setMedalOpen(false);
      setMedalTarget(null);
      await loadAll();
    } catch {
      toast.error("Could not award medal");
    } finally {
  ...
  const deleteListing = async (listingId) => {
    ...
    try {
      ...
      toast.success("Listing deleted by owner");
      await loadAll();
    } catch {
...
<div className="text-xs font-tech uppercase tracking-[0.3em] text-muted-foreground mb-3 rise-in">Owner Control</div>
                  ...
                  <td>{u.terms_enforcement_status}</td>
                   <td>
                     <div className="flex flex-wrap gap-1">
                       ...
                       <Button size="sm" variant="outline" className="h-7 text-[10px]" onClick={() => issueWarning(u.user_id)}>Issue Warning</Button>
                       <Button size="sm" variant="outline" className="h-7 text-[10px]" onClick={() => openSuspend(u)}>Suspend</Button>
                       <Button size="sm" variant="outline" className="h-7 text-[10px]" onClick={() => openMedal(u)}><Award className="h-3 w-3 mr-1" /> Medal</Button>
                       <Button size="sm" className="h-7 text-[10px] bg-destructive hover:bg-destructive/90" onClick={() => terminateProfile(u.user_id)}>Terminate</Button>
                     </div>
                   </td>
                </tr>
              ))}
      ...
      <section className="border border-border rounded-xl p-4 bg-card mb-6">
        <h2 className="font-tech text-xs uppercase tracking-wider text-muted-foreground mb-3">Policy Management</h2>
        <p className="text-sm text-muted-foreground mb-4">Edit Terms of Service and Privacy Policy with icon selection for each rule.</p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <a href="/owner/policy/terms" className="block border border-border rounded-xl p-4 hover:border-primary/50 hover:bg-primary/5 transition-colors">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Manage platform terms, seller rules, and user agreements</p>
              </div>
            </div>
          </a>
          <a href="/owner/policy/privacy" className="block border border-border rounded-xl p-4 hover:border-primary/50 hover:bg-primary/5 transition-colors">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 flex items-center justify-center bg-primary/10 rounded-lg">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Manage data collection, cookies, and privacy disclosures</p>
              </div>
            </div>
          </a>
        </div>
      ...
      <Dialog open={suspendOpen} onOpenChange={(open) => { setSuspendOpen(open); if (!open) setSuspendTarget(null); }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="font-display text-xl flex items-center gap-2"><Ban className="h-5 w-5 text-destructive" /> Suspend User</DialogTitle>
          </DialogHeader>
          {suspendTarget && (
            <div className="space-y-4">
              <div className="text-sm text-muted-foreground">
                Suspending <span className="text-foreground font-medium">{suspendTarget.name}</span> ({suspendTarget.user_tag || suspendTarget.user_id})
              </div>
              <div>
              ...
              <div>
                <label className="text-[10px] uppercase tracking-wider font-tech text-muted-foreground mb-1 block">Reason</label>
                <Textarea rows={3} value={suspendReason} onChange={(e) => setSuspendReason(e.target.value)} placeholder="Policy violation reason..." />
              </div>
            </div>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setSuspendOpen(false)} className="rounded-xl font-tech text-xs uppercase tracking-wider">Cancel</Button>
            <Button onClick={suspendUser} disabled={suspendSubmitting} className="rounded-xl font-tech text-xs uppercase tracking-wider bg-destructive hover:bg-destructive/90">
      ...
      <Dialog open={medalOpen} onOpenChange={(open) => { setMedalOpen(open); if (!open) setMedalTarget(null); }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="font-display text-xl flex items-center gap-2"><Award className="h-5 w-5 text-accent" /> Award Exclusive Medal</DialogTitle>
          </DialogHeader>
          {medalTarget && (
            <div className="space-y-4">
              <div className="text-sm text-muted-foreground">
                Awarding to <span className="text-foreground font-medium">{medalTarget.name}</span> ({medalTarget.user_tag || medalTarget.user_id})
              </div>
              <div>
                <label className="text-[10px] uppercase tracking-wider font-tech text-muted-foreground mb-1 block">Badge ID</label>
                <Input value={medalId} onChange={(e) => setMedalId(e.target.value)} placeholder="e.g. custom_hero_medal" />
              </div>
              <div>
                <label className="text-[10px] uppercase tracking-wider font-tech text-muted-foreground mb-1 block">Label</label>
                <Input value={medalLabel} onChange={(e) => setMedalLabel(e.target.value)} placeholder="e.g. Community Hero" />
              </div>
              <div>
                <label className="text-[10px] uppercase tracking-wider font-tech text-muted-foreground mb-1 block">Description</label>
                <Textarea rows={2} value={medalDesc} onChange={(e) => setMedalDesc(e.target.value)} placeholder="Short description..." />
              </div>
            </div>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setMedalOpen(false)} className="rounded-xl font-tech text-xs uppercase tracking-wider">Cancel</Button>
            <Button onClick={awardMedal} disabled={medalSubmitting} className="rounded-xl font-tech text-xs uppercase tracking-wider">
              {medalSubmitting ? "Awarding..." : "Award Medal"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
